...
var server = http.createServer(function (request, response) {
    ...
    console.log('有一个请求发送过来啦！路径（带查询参数）为：' + pathWithQuery)
    ...
    if (path === '/index.html') {
        ...
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/javascript;charset=utf-8')
        response.write(fs.readFileSync('public/js1.js'))
        response.end()
        ...
        response.end()
    } else if (path === '/xml1.xml') {
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/xml;charset=utf-8')
        response.write(fs.readFileSync('public/xml1.xml'))
        response.end()
    } else if (path === '/page.json') {
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/json;charset=utf-8')
        response.write(fs.readFileSync('public/page.json'))
        response.end()
